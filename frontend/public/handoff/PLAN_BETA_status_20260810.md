...
- **P3 backlog · Solicitar a Intel `shareholder.type ∈ {legal, individual}` explícito** en el shape de `ownership.shareholders[]` para habilitar futura discriminación de PII en vista anónima con menor pérdida de valor CF (permitiría mostrar nombres/pcts de personas jurídicas — dato registral público — mientras se anonimizan personas físicas). Bloqueo actual: Intel no emite `type` y los CIFs llegan `null` para sociedades extranjeras (Servier BV, sociedades francesas), imposibilitando heurística fiable. Emitir REQ formal en `/_requirements_for_agency_tool/` en próxima ronda de escalación Intel.
- **Fases Events shell, Mercado, resto B-3/B-4 en espera de Intel I-2/I-3/I-4** (events ya llega vía `ficha.events`; UI shell pendiente).
