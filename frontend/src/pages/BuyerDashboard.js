...
const BuyerDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const statsRes = await marketplaceAPI.getStats();
        setStats(statsRes.data);

        // Fetch recommended deals from matching engine
        try {
  ...
  const quickActions = [
    { icon: Search, title: 'Explorar Deals', description: 'Busca oportunidades de inversión', href: '/marketplace', color: 'arroba-coral' },
    { icon: FileText, title: 'Mis Procesos', description: 'Deals en los que participas', href: '/buyer/processes', color: 'arroba-blue' },
    { icon: Bell, title: 'Alertas', description: 'Configura tus notificaciones', href: '/buyer/alerts', color: 'arroba-yellow' },
  ];
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8" data-testid="buyer-dashboard">
        ...
        {/* Profile Completion Alert — only if profile not complete */}
        {!profileComplete && (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Sin perfil completo no podrás enviar interés, LOI ni ver recomendaciones
                  </p>
                </div>
              </div>
              <Link to="/buyer/onboarding">
                <Button className="bg-arroba-coral hover:bg-arroba-coral/90 text-white" data-testid="complete-profile-btn">
                  COMPLETAR PERFIL
                </Button>
              </Link>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            {loading ? (
              ...
              <div className="card-arroba text-center py-8" data-testid="no-recommendations">
                ...
                <p className="text-sm text-slate-500 mb-4">
                  Completa tu perfil de comprador para activar el motor de matching y recibir recomendaciones personalizadas
                </p>
                <Link to="/buyer/onboarding">
              ...
              <div className="card-arroba text-center py-8" data-testid="no-deals-found">
                ...
                <p className="text-sm text-slate-500 mb-4">
                  No encontramos deals que coincidan con tu perfil. Te avisaremos cuando haya nuevas oportunidades.
                </p>
                <Link to="/marketplace">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            ...
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase flex items-center gap-1 ${aff.class}`} data-testid={`affinity-badge-${deal.deal_id}`}>
                              {AffIcon && <AffIcon className="w-3 h-3" />}
                              {aff.label}
                            </span>
                          </div>
                          ...
                          <p className="text-sm text-slate-500 line-clamp-1">
                            {teaser.short_description || teaser.description || ''}
                          </p>
          ...
          <div className="space-y-6">
            <div className="card-arroba">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-500">Deals vistos</span><span className="font-bold">0</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-500">NDAs firmados</span><span className="font-bold">0</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-500">LOIs enviadas</span><span className="font-bold">0</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-500">Procesos activos</span><span className="font-bold">0</span>
                </div>
              </div>
            </div>
            ...
            <div className="card-arroba bg-slate-50">
              ...
              <Link to="/pricing">
                <Button className="w-full btn-primary">VER PLANES</Button>
              </Link>
            </div>
          </div>
        </div>
