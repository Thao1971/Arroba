...
function EvolutionChart({ series }) {
  ...
  const maxM = Math.max(...margen) * 1.25;
  const step = cW / n;
  const gap = step * 0.16;
  ...
  const yMar = v => padT + cH - (v / maxM) * cH;
  const mPts = margen.map((m, i) => ({ x: padL + i * step + step / 2, y: yMar(m), v: m }));
  const lineD = mPts.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ');
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible', display: 'block' }}>
      ...
      {[0.25, 0.5, 0.75, 1].map(f => (
        <text key={f} x={W - padR + 6} y={yMar(maxM * f) + 4} fontSize={9} fill="var(--text-subtle)">{Math.round(maxM * f)}%</text>
      ))}
    </svg>
  );
}
