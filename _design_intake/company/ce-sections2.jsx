...
function OwnerNode({ name, sub, pct, color, kind, highlight }) {
  return (
    <div style={{ position: 'relative', background: 'var(--surface)', border: `1.5px solid ${highlight ? '#E8001D' : 'var(--border)'}`, borderRadius: 11, padding: '12px 14px', minWidth: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
        <span style={{ width: 9, height: 9, borderRadius: kind === 'persona' ? '50%' : 2, background: color, flexShrink: 0 }}></span>
        <span style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</span>
      </div>
      ...
      <CECard pad={24}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
          {pa.map((p, i) => {
            ...
            return (
              <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 11, padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--text)', lineHeight: 1.2 }}>{p.name}</span>
                </div>
...
Object.assign(window, { SecPropiedad, SecGobierno, SecMercado, SecTransacciones, SIG_C });
