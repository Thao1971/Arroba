// arroba.com — Investor Intelligence · app

function IVApp() {
  const D = window.IV_DATA;
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      <style>{`
        * { transition: background-color .15s, border-color .12s, color .1s; }
        ...
        .iv-chip:hover { border-color: var(--border-strong) !important; transform: translateY(-1px); }
        .iv-opp:hover { border-color: #E8001D !important; transform: translateY(-2px); }
        .iv-row:hover { background: var(--surface-2) !important; }
        @media (max-width: 880px) {
          .iv-kpis, .iv-opps, .iv-panels { grid-template-columns: 1fr !important; }
          .iv-nav-links { display: none !important; }
        }
      `}</style>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 300, background: 'color-mix(in srgb, var(--bg) 88%, transparent)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', borderBottom: '1px solid var(--border)', height: 72, display: 'flex', alignItems: 'center', padding: '0 28px', gap: 28 }}>
        ...
        <div className="iv-nav-links" style={{ display: 'flex', gap: 4 }}>
      ...
      <div style={{ maxWidth: 1020, margin: '0 auto', padding: '0 28px 64px' }}>
        ...
        <div style={{ textAlign: 'center', paddingTop: 56, marginBottom: 36 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderRadius: 999, background: 'var(--surface)', border: '1px solid var(--border)', marginBottom: 26 }}>
            ...
            <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text-muted)' }}>Investor Intelligence · España</span>
          </div>
          <h1 style={{ fontSize: 'clamp(34px, 5vw, 50px)', fontWeight: 700, fontFamily: 'var(--font-display)', color: 'var(--text)', letterSpacing: '-.025em', lineHeight: 1.08, marginBottom: 18 }}>
            Hola, <span style={{ color: '#E8001D' }}>{D.user}</span><br/>¿Qué inversor estás buscando?
          </h1>
          <p style={{ fontSize: 16.5, color: 'var(--text-muted)', lineHeight: 1.55, maxWidth: 600, margin: '0 auto' }}>
            Identifica fondos, family offices e inversores activos en España, entiende su tesis y descubre quién encaja con tu compañía o tu oportunidad.
          </p>
        </div>
        ...
        <div style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 8px 8px 20px', background: 'var(--surface)', border: `1.5px solid ${focus ? '#E8001D' : 'var(--border-strong)'}`, borderRadius: 16, boxShadow: focus ? '0 6px 28px rgba(232,0,29,.12)' : '0 6px 28px rgba(12,12,14,.06)' }}>
            ...
            <input value={query} onChange={e => setQuery(e.target.value)} onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}
              ...
              placeholder="Buscar fondos, family offices o inversores…"
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9, justifyContent: 'center', marginBottom: 22 }}>
          {D.chips.map(c => (
            <button key={c.label} className="iv-chip" onClick={go} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '9px 15px', borderRadius: 999, background: 'var(--surface)', border: '1px solid var(--border)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: 'var(--text)', transition: 'border-color .12s, transform .12s' }}>
        ...
        <div style={{ marginBottom: 44 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
            {D.recent.map(r => (
              <button key={r} className="iv-chip" onClick={go} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '9px 15px', borderRadius: 999, background: 'var(--surface)', border: '1px solid var(--border)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', transition: 'border-color .12s, transform .12s' }}>
        ...
        {/* ── KPIs ── */}
        <div className="iv-kpis" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 18 }}>
          {D.kpis.map(k => (
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '22px 24px', marginBottom: 28 }}>
          ...
          <div className="iv-opps" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }}>
            {D.opportunities.map((o, i) => (
              <a key={o.title} href="Universal Search.html" className="iv-opp" style={{ display: 'block', textDecoration: 'none', padding: '4px 18px', borderLeft: i ? '1px solid var(--border)' : 'none', borderRadius: 8, transition: 'border-color .12s' }}>
        ...
        {/* ── PANELES ── */}
        <div className="iv-panels" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--border)', padding: '22px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 18 }}>
              ...
              <span style={{ fontSize: 15.5, fontWeight: 700, color: 'var(--text)', fontFamily: 'var(--font-display)', marginLeft: 9 }}>Inversores más activos</span>
              <a href="Universal Search.html" style={{ marginLeft: 'auto', fontSize: 12.5, fontWeight: 600, color: '#E8001D', textDecoration: 'none', whiteSpace: 'nowrap' }}>Ver todos →</a>
            </div>
            ...
            {D.investors.map((s, i) => (
              <a key={s.name} href="Company Profile.html" className="iv-row" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '11px 8px', margin: '0 -8px', borderRadius: 9, textDecoration: 'none', transition: 'background .12s' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>{s.type}</div>
                </div>
                <div style={{ width: 92, height: 6, background: 'var(--surface-2)', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: s.value + '%', background: '#1A8A4A', borderRadius: 3 }}/>
                </div>
                <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', fontVariantNumeric: 'tabular-nums', width: 26, textAlign: 'right' }}>{s.ops}</span>
              </a>
            ))}
            <div style={{ fontSize: 11, color: 'var(--text-subtle)', lineHeight: 1.5, marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
              Inversores ordenados por número de operaciones registradas en los últimos 12 meses.
            </div>
          </div>

          {/* Últimas operaciones de inversión */}
          <div style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--border)', padding: '22px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 18 }}>
              <HIcon name="euro" size={16} color="#E8001D" sw={1.75}/>
              <span style={{ fontSize: 15.5, fontWeight: 700, color: 'var(--text)', fontFamily: 'var(--font-display)', marginLeft: 9 }}>Últimas operaciones de inversión</span>
            ...
            {D.deals.map(c => (
              <a key={c.name} href="Oportunidad.html" className="iv-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 8px', margin: '0 -8px', borderRadius: 9, textDecoration: 'none', transition: 'background .12s' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-subtle)' }}>{c.round} · {c.investor}</div>
                </div>
                <div style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                  <div style={{ fontSize: 13.5, fontWeight: 700, color: '#E8001D', fontVariantNumeric: 'tabular-nums' }}>{c.amount}</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, marginTop: 36, fontSize: 12.5, color: 'var(--text-muted)' }}>
          <span style={{ position: 'relative', display: 'inline-flex', width: 8, height: 8 }}>
            <span style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: '#1A8A4A', animation: 'ivLive 1.8s ease-out infinite' }}></span>
            <span style={{ position: 'relative', width: 8, height: 8, borderRadius: '50%', background: '#1A8A4A' }}></span>
          </span>
          <span><strong style={{ color: 'var(--text)', fontVariantNumeric: 'tabular-nums' }}>{D.footerCount}</strong> inversores con inteligencia activa y tesis cruzadas con el mercado</span>
        </div>
      </div>
    </div>
  );
...
ReactDOM.createRoot(document.getElementById('root')).render(<IVApp/>);
