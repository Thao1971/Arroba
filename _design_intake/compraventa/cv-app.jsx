// arroba.com — Home de Compra/Vende · app
...
function CVApp() {
  ...
  const TAG = {
    venta:   { c: '#1A8A4A', bg: '#E8F5EE', bd: '#C2E8D0' },
    compra:  { c: '#2164E3', bg: '#E8EFFE', bd: '#C0D3FA' },
    capital: { c: '#D97708', bg: '#FEF3E2', bd: '#FCD9A3' },
  };
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      <style>{`
        * { transition: background-color .15s, border-color .12s, color .1s; }
        ...
        .cv-chip:hover { border-color: var(--border-strong) !important; transform: translateY(-1px); }
        .cv-opp:hover { border-color: #E8001D !important; transform: translateY(-2px); }
        .cv-row:hover { background: var(--surface-2) !important; }
        @media (max-width: 880px) {
          .cv-kpis, .cv-opps, .cv-panels { grid-template-columns: 1fr !important; }
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 300, background: 'color-mix(in srgb, var(--bg) 88%, transparent)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', borderBottom: '1px solid var(--border)', height: 60, display: 'flex', alignItems: 'center', padding: '0 28px', gap: 28 }}>
        ...
        <div className="cv-nav-links" style={{ display: 'flex', gap: 4 }}>
          {[['Analiza', 'Analiza.html'], ['Valora', 'Valora.html'], ['Compra/Vende', '#']].map(([l, href]) => {
            const active = l === 'Compra/Vende';
            return (
      ...
      <div style={{ maxWidth: 1020, margin: '0 auto', padding: '0 28px 64px' }}>
        ...
        <div style={{ textAlign: 'center', paddingTop: 56, marginBottom: 36 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderRadius: 999, background: 'var(--surface)', border: '1px solid var(--border)', marginBottom: 26 }}>
            ...
            <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text-muted)' }}>Inteligencia transaccional · España</span>
          </div>
          <h1 style={{ fontSize: 'clamp(34px, 5vw, 50px)', fontWeight: 700, fontFamily: 'var(--font-display)', color: 'var(--text)', letterSpacing: '-.025em', lineHeight: 1.08, marginBottom: 18 }}>
            Hola, <span style={{ color: '#E8001D' }}>{D.user}</span><br/>¿Qué quieres comprar o vender?
          </h1>
          <p style={{ fontSize: 16.5, color: 'var(--text-muted)', lineHeight: 1.55, maxWidth: 600, margin: '0 auto' }}>
            Encuentra empresas en venta, identifica compradores e inversores compatibles y activa oportunidades de compra, venta o captación de capital.
          </p>
        </div>
        ...
        <div style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 8px 8px 20px', background: 'var(--surface)', border: `1.5px solid ${focus ? '#E8001D' : 'var(--border-strong)'}`, borderRadius: 16, boxShadow: focus ? '0 6px 28px rgba(232,0,29,.12)' : '0 6px 28px rgba(12,12,14,.06)' }}>
            ...
            <input value={query} onChange={e => setQuery(e.target.value)} onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}
              ...
              placeholder="Buscar empresas en venta, compradores o inversores…"
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9, justifyContent: 'center', marginBottom: 22 }}>
          {D.chips.map(c => (
            <button key={c.label} className="cv-chip" onClick={() => go(c.label)} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '9px 15px', borderRadius: 999, background: 'var(--surface)', border: '1px solid var(--border)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: 'var(--text)', transition: 'border-color .12s, transform .12s' }}>
        ...
        <div style={{ marginBottom: 44 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
            {D.recent.map(r => (
              <button key={r} className="cv-chip" onClick={() => go(r)} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '9px 15px', borderRadius: 999, background: 'var(--surface)', border: '1px solid var(--border)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', transition: 'border-color .12s, transform .12s' }}>
        ...
        {/* ── KPIs DEL MERCADO ── */}
        <div className="cv-kpis" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 18 }}>
          {D.kpis.map(k => (
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '22px 24px', marginBottom: 28 }}>
          ...
          <div className="cv-opps" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }}>
            {D.opportunities.map((o, i) => (
              <a key={o.title} href="Universal Search.html" className="cv-opp" style={{ display: 'block', textDecoration: 'none', padding: '4px 18px', borderLeft: i ? '1px solid var(--border)' : 'none', borderRadius: 8, transition: 'border-color .12s' }}>
        ...
        {/* ── PANELES INFERIORES ── */}
        <div className="cv-panels" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '22px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 18 }}>
              <HIcon name="deal" size={17} color="#E8001D" sw={1.75}/>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 11, fontWeight: 600, letterSpacing: '.04em', textTransform: 'uppercase', color: 'var(--text-subtle)', paddingBottom: 10, borderBottom: '1px solid var(--border)', marginBottom: 4 }}>
              <span>Sector</span>
              <span>Operaciones (24m)</span>
            </div>
            {D.sectors.map((s, i) => (
              <a key={s.name} href="Ficha Sectorial.html" className="cv-row" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '11px 8px', margin: '0 -8px', borderRadius: 9, textDecoration: 'none', transition: 'background .12s' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-subtle)', lineHeight: 1.5, marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
              Número de operaciones de compraventa registradas por sector en los últimos 24 meses.
            </div>
          </div>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '22px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 18 }}>
              ...
              <span style={{ fontSize: 15.5, fontWeight: 700, color: 'var(--text)', fontFamily: 'var(--font-display)', marginLeft: 9 }}>Oportunidades recientes</span>
            ...
            {D.deals.map(c => {
              const t = TAG[c.tagType];
              return (
                <a key={c.name} href="Oportunidad.html" className="cv-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 8px', margin: '0 -8px', borderRadius: 9, textDecoration: 'none', transition: 'background .12s' }}>
                  ...
                  <div style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                    <span style={{ fontSize: 11, fontWeight: 600, color: t.c, background: t.bg, border: `1px solid ${t.bd}`, padding: '3px 9px', borderRadius: 6 }}>{c.tag}</span>
                    <div style={{ fontSize: 11.5, color: 'var(--text-subtle)', marginTop: 4 }}>{c.when}</div>
                  </div>
                </a>
              );
            })}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, marginTop: 36, fontSize: 12.5, color: 'var(--text-muted)' }}>
          <span style={{ position: 'relative', display: 'inline-flex', width: 8, height: 8 }}>
            <span style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: '#1A8A4A', animation: 'cvLive 1.8s ease-out infinite' }}></span>
            <span style={{ position: 'relative', width: 8, height: 8, borderRadius: '50%', background: '#1A8A4A' }}></span>
          </span>
          <span><strong style={{ color: 'var(--text)', fontVariantNumeric: 'tabular-nums' }}>{D.footerCount}</strong> oportunidades activas en el mercado, con compradores e inversores compatibles</span>
        </div>
      </div>
    </div>
  );
