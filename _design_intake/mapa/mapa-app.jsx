// arroba.com — Mapa empresarial app (public top nav like Home)
...
function MapaNav() {
  ...
  return (
    <nav style={{
      ...
      {/* Universal AI search */}
      <div style={{ flex: 1, maxWidth: 460, margin: '0 24px', display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px', background: 'var(--surface)', border: '1.5px solid var(--border-strong)', borderRadius: 10, cursor: 'pointer' }}
        onMouseEnter={e => { e.currentTarget.style.borderColor = '#E8001D'; }}
        onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border-strong)'; }}>
        <span style={{ fontSize: 15, color: '#E8001D' }}>✦</span>
        <span style={{ fontSize: 13, color: 'var(--text-subtle)', flex: 1 }}>Busca provincias, sectores, territorios...</span>
...
function MapaApp() {
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      {/* Content */}
      <div style={{ padding: '28px 32px 60px', maxWidth: 1380, margin: '0 auto' }}>
        {/* Breadcrumb */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 12, color: 'var(--text)', fontWeight: 600 }}>Mapa empresarial</span>
        </div>
        ...
        {/* Main 3-column: filters / map / ranking */}
        <div style={{ display: 'grid', gridTemplateColumns: '230px 1fr 340px', gap: 16, marginBottom: 16, alignItems: 'start' }}>
          <FiltersPanel/>
          <MapPanel/>
          <RankingPanel/>
        </div>

        {/* Bottom row: evolution / drive / emerging / opp */}
        <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr 1fr 1.3fr', gap: 16, marginBottom: 16, alignItems: 'start' }}>
          <EvolutionCard/>
          <DriveSectorsCard/>
          <EmergingSectorsCard/>
          <OppSectorTerritoryCard/>
        </div>

        {/* Comparator + sources */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16, marginBottom: 8, alignItems: 'start' }}>
          <ComparatorCard/>
          <SourcesCard/>
        </div>
...
ReactDOM.createRoot(document.getElementById('root')).render(<MapaApp/>);
