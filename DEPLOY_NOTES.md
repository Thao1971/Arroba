...
| HARDENING-030c   | **Bundle 28.1 (hotfix intra-bundle)**. Gráfico de UN ejercicio con fuente en cascada. Extraído helper puro `resolveSingleExerciseCascade(legacy, evoPoints)` en `SingleExerciseChart.tsx` con doble prioridad: (1) `singleExerciseFromEvolution(financial.evolution)` legacy (contrato preservado, `ebitdaMargin=null`); (2) fallback al **único punto** del agregador `financialAnalysis.evolution.points[0]` con `{year, revenue, ebitda, ebita_margin}`. Cascada aplicada vía `useMemo` en `CompanyFichaLayoutV2.tsx` (line 604). R15 estricto: si ni legacy trae 1 año ni el agregador tiene exactamente 1 punto → `null` (layout renderiza `<Pending/>`). 7 tests unitarios en `SingleExerciseChart.test.ts` cubren los 3 escenarios (legacy con datos · legacy vacía + agregador con 1 punto · ambos vacíos) + edge cases (año no numérico, ≥2 puntos, campos coercibles). Verificación preview: Servier B28184687 (3 puntos agregador) → hotfix devuelve `null` → chart tendencia (regresión OK); B60418605 preview (Intel devuelve `evolution.points: []` con `years=1`) → hotfix devuelve `null` → `<Pending/>` (comportamiento correcto). Bug reproducible **solo en prod** donde Intel puebla el único punto. Bundle sigue en **28 unidades** consolidadas (hotfix intra-bundle). | ✅ |
