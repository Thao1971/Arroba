...
- **P2 · REQ-INTEL emitido · `PARA_INTEL_financial_quality_verdict.md`** (2026-08-11) · propagación del campo `verdict` en la proyección `financial_quality` del agregador `/company/{cif}/ficha`. Verificación E2E confirmó que Intel posee el campo en su modelo interno pero **no lo incluye en la proyección** (7 CIFs muestra Turno D: `verdict` ausente). Card "Veredicto de ARROBA" del Hero permanece `<Empty/>` "Información en preparación" hasta armonización Intel · TODO comment in situ en `HeroBlock` líneas 312-317. Descargable en `/handoff/PARA_INTEL_financial_quality_verdict.md`.
