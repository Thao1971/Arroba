# arroba.com — Entity Framework v1.1.0
...
> - Catálogo de tipos abstraído (sin lenguaje de cardinalidad). Se enumeran los tipos sin cuantificarlos.
> - **`Match` pasa a ser una entidad canónica de primer nivel** (§11.13 nueva): elevado desde el modelo relacional a tipo con ficha, anatomía propia y reglas de composición. Resuelve la decisión G2.M (`OPEN_ITEMS_CLASSIFICATION.md` §2).
> - Renombre `client → user` para alinearse con la nomenclatura canónica de actor humano del producto (`ENTITY_MODEL.md` §3, §5.1).
> - Sustitución de `team_arroba → arroba_team` donde aplicaba (cero ocurrencias residuales en este documento).
> - §11.8 Operación actualizado con el orden canónico de `current_phase`: `nda → im → qa → loi → dd → negotiation → spa → closing → integration` (resuelve G2.O).
> - Numeración de fases del journey de transacción referida como `T1`…`T15` (sin re-enumerar; ver `TRANSACTION_OS_SPEC.md` §3).
> - §9.2 / §9.3 / §11 ampliadas para reflejar `match` y el renombre `user`.
>
...
3. [Anatomía canónica — los módulos](#3-anatomía-canónica--los-módulos)
...
- **NO es un sistema de plantillas configurables por el usuario** — los
  módulos y su orden son canónicos. El usuario no reordena.
...
### Tipos canónicos (resumen)

> Catálogo abierto y declarativo. El detalle ontológico vive en `ENTITY_MODEL.md` §3. Aquí se listan los tipos con ficha UX propia, sin cuantificarlos: el catálogo evoluciona por declaración, no por cardinalidad.
...
| `match` | `/match/{id}` | Match Kitchen ↔ Buy-side SaaS Horeca |
| `user` | (sin ficha pública) | usuario interno |
| `organization` | `/org/{slug}` | ARROBA Demo Org |

`match` pasa a ser una entidad canónica de primer nivel (anatomía propia en §11.13). `user` es el renombre canónico del antiguo `client`.

Detalle ontológico completo en `ENTITY_MODEL.md` §3.

---

## 3. Anatomía canónica — los módulos

Toda ficha de entidad se compone declarativamente con un conjunto canónico de módulos. Cada módulo es un concepto UX, no un componente React específico (los componentes viven en `src/components/entity/base/`, y la traducción al Design System está en `DESIGN_SYSTEM.md` §2).
...
| `match` | Header · KPIs (score, fit, fase) · Insights (razones del match) · Relaciones (origen ↔ destino) · Actividad |

### 9.3 Módulos prohibidos por tipo

| Tipo | Prohibidos |
|---|---|
| `user` | (no tiene ficha pública) |
| `document` | KPIs, Análisis (un PDF no se analiza con NarrativeBlock) |
| `person` | KPIs (no es el lenguaje canónico de una persona) |
| `match` | Hero, Documentación (un match es un objeto relacional, no narrativo ni documental) |
...
> `current_phase` sigue el orden canónico **`nda → im → qa → loi → dd → negotiation → spa → closing → integration`** (ver `ENTITY_MODEL.md` §5.8 y `TRANSACTION_OS_SPEC.md` §3). El Header refleja la fase actual y, según la fase, restringe la visibilidad de campos sensibles (target anonimizado pre-NDA).

| # | Módulo | Activo | Notas |
|---|---|---|---|
| 1 | Header | ✅ | empresa target (anonimizada si pre-NDA) + fase actual (`nda`…`integration`) + counterparts visible tras NDA firmado |
| 2 | Hero | 🔵 opcional | resumen breve del proceso |
| 3 | KPIs | ✅ | días en fase actual · contrapartes activas · documentos pendientes · fase del journey (`T1`…`T15`) |
| 4 | Advisor | ✅ | "✦ Deal Advisor" |
| 5 | Insights | ✅ | siguientes pasos críticos por fase |
| 6 | Análisis | 🔵 opcional | due diligence narrativa (activo en fases `dd`, `negotiation`) |
| 7 | Señales | ✅ | eventos relevantes (NDA firmado, IOI recibido, milestone QA) |
| 8 | Relaciones | ✅ | mandato origen, match origen (si nació de uno), valoraciones aplicadas |
| 9 | Oportunidades | ❌ omitido | |
| 10 | Documentación (CENTRAL) | ✅ | organizada por fase canónica: NDA → IM → QA → LOI → DD → SPA → Closing → Integration |
| 11 | Actividad | ✅ | timeline detallado con transiciones de fase explícitas |
| 12 | Acciones | ✅ | "Avanzar fase", "Invitar contraparte", "Abrir Data Room" |

---

### 11.13 Match (`match`) — 🔵 backlog

> **Decisión canónica**: Match pasa a ser una entidad canónica de primer nivel (decisión G2.M; ver `OPEN_ITEMS_CLASSIFICATION.md` §2 y `ENTITY_MODEL.md` §5.7). Un match es un objeto relacional con identidad propia, ciclo de vida observable, score, fase y trazabilidad bidireccional entre origen y destino. Materializa el resultado del motor de matching como una entidad navegable.

**Anatomía**:

| # | Módulo | Activo | Notas |
|---|---|---|---|
| 1 | Header | ✅ | título compuesto "`{origen} ↔ {destino}`"; iconos por tipo de extremo; fase del match (`detected → reviewed → contacted → engaged → converted_to_operation → discarded`) |
| 2 | Hero | ❌ prohibido | un match no necesita storytelling; el header con score basta |
| 3 | KPIs | ✅ | score global · fit por dimensión (sectorial, financiero, geográfico, estratégico) · días desde detección |
| 4 | Advisor | ✅ | "✦ Match Advisor" — Copilot que justifica el score y propone acción |
| 5 | Insights | ✅ | razones del match (bullets tipados `signal`, `strength`, `fit_factor`) |
| 6 | Análisis | 🔵 opcional | narrative cualitativa solo en matches relevantes (alto score) |
| 7 | Señales | 🔵 opcional | señales que detonaron el match (BORME, contratación, etc.) |
| 8 | Relaciones | ✅ | extremo origen (mandato, oportunidad o empresa) · extremo destino · operación derivada si existe |
| 9 | Oportunidades | ❌ omitido | un match ES la relación, no contiene oportunidades |
| 10 | Documentación | ❌ prohibido | no aplica |
| 11 | Actividad | ✅ | transiciones de fase, contactos lanzados, decisiones del usuario |
| 12 | Acciones | ✅ | "Convertir en operación", "Descartar", "Solicitar más información", "Notificar al equipo" |

**Especializaciones del módulo Header para Match**:
- Discriminador `match_type` (`mandate↔company`, `opportunity↔company`, `company↔company`) define los iconos.
- Score badge prominente (de 0 a 100) con código de color por tramo.

**Estados típicos**:
- `detected` (recién creado por el engine) → `reviewed` (un humano lo ha visto) → `contacted` (se ha lanzado contacto) → `engaged` (contraparte respondió) → `converted_to_operation` (se creó una `operation`) o `discarded` (motivo registrado).

**Por qué entidad de primer nivel**: un match con identidad propia permite auditoría (¿por qué se descartó?), reactivación (revisar matches descartados ante cambios de contexto), métricas (conversión matches→operaciones), y trazabilidad bidireccional cuando una operación nace de un match.

---

## 12. Principios de evolución

### 12.1 Añadir un nuevo módulo

Requiere:
1. Justificación en `ARROBA_PHILOSOPHY.md` (qué patrón nuevo no encaja
   en los módulos actuales).
2. Sección 3.x nueva en este documento con propósito + contenido +
   cuándo aparece + cuándo se omite + especialización.
3. Lugar en el orden canónico §4 (nueva posición declarada).
...
Cambiar el orden de los módulos actuales requiere consenso explícito; rompe la
