...
from src.modules.intelligence_layer.providers.agency_tool.signal import (
    AgencyToolSignalProvider,
)
from src.modules.intelligence_layer.providers.mock.signal import MockSignalProvider
...
from src.modules.intelligence_layer.providers.agency_tool.recommendation import (
    AgencyToolRecommendationProvider,
)
from src.modules.intelligence_layer.providers.mock.recommendation import (
...
class IntelligenceRouter:
    ...
    def __init__(
        ...
        *,
        ...
        self._signal_provider: SignalProvider | None = None
        self._recommendation_provider: RecommendationProvider | None = None
    ...
    # ---------- Proveedor Signal (§6.4) ----------
    def _get_signal_provider(self) -> SignalProvider:
        if self._signal_provider is None:
            if self.settings.agency_tool_mode == "real":
                self._signal_provider = AgencyToolSignalProvider()
            else:
                self._signal_provider = MockSignalProvider()
        return self._signal_provider

    # ---------- Proveedor Recommendation (§6.6) ----------
    def _get_recommendation_provider(self) -> RecommendationProvider:
        if self._recommendation_provider is None:
            if self.settings.agency_tool_mode == "real":
                self._recommendation_provider = AgencyToolRecommendationProvider()
            else:
                self._recommendation_provider = MockRecommendationProvider()
    ...
    async def get_signal_analysis(self, cif: str) -> SignalAnalysis:
        """`POST /signal-intelligence/analyze` con caché + breaker + métricas."""
        return SignalAnalysis.model_validate(
            await self._call_signal(identifier=cif.upper())
        )

    async def _call_signal(self, *, identifier: str) -> dict[str, Any]:
        provider = self._get_signal_provider()
        engine = "signal"
        provider_name = provider.provider_name
        ttl = self.settings.ttl_for("signal")
        breaker = self._get_breaker(provider_name, engine)
        cache_key = build_key(
            provider=provider_name, engine=engine, method="analyze", master_id=identifier
        )
        ...
        async def _compute() -> dict[str, Any]:
            ...
            try:
                ...
                requests_total.labels(
                    provider=provider_name, engine=engine, method="analyze", status="breaker_open"
                ).inc()
                raise
            except SignalNotFoundError:
                requests_total.labels(
                    provider=provider_name, engine=engine, method="analyze", status="not_found"
                ).inc()
                raise
                ...
                requests_total.labels(
                    provider=provider_name, engine=engine, method="analyze", status="error"
                ).inc()
                raise
            finally:
                request_duration_seconds.labels(
                    provider=provider_name, engine=engine, method="analyze"
                ).observe(time.monotonic() - start)

            requests_total.labels(
                provider=provider_name, engine=engine, method="analyze", status="ok"
            ).inc()
            return record.model_dump(mode="json")
    ...
    # ================================================================
    # Recommendation Engine (§6.6)
    # ================================================================

    async def get_buyers(self, cif: str, limit: int = 10) -> RecommendationSet:
        """`POST /recommendation-intelligence/buyers` con caché + breaker + métricas."""
        return RecommendationSet.model_validate(
            await self._call_recommendation(op="buyers", identifier=cif.upper(), limit=limit)
        )

    async def get_opportunities(self, cif: str, limit: int = 10) -> RecommendationSet:
        """`POST /recommendation-intelligence/opportunities` con caché + breaker + métricas."""
        return RecommendationSet.model_validate(
            await self._call_recommendation(op="opportunities", identifier=cif.upper(), limit=limit)
        )

    async def _call_recommendation(self, *, op: str, identifier: str, limit: int) -> dict[str, Any]:
        provider = self._get_recommendation_provider()
        engine = "recommendation"
        provider_name = provider.provider_name
        ttl = self.settings.ttl_for("recommendation")
        breaker = self._get_breaker(provider_name, engine)
        cache_key = build_key(
            provider=provider_name, engine=engine, method=f"{op}:{limit}", master_id=identifier
        )
        ...
        async def _compute() -> dict[str, Any]:
            ...
            async def _invoke():
                if op == "opportunities":
                    return await provider.opportunities(identifier, limit)
                return await provider.buyers(identifier, limit)
            ...
            try:
                ...
                requests_total.labels(
                    provider=provider_name, engine=engine, method=op, status="breaker_open"
                ).inc()
                raise
            except RecommendationNotFoundError:
                requests_total.labels(
                    provider=provider_name, engine=engine, method=op, status="not_found"
                ).inc()
                raise
                ...
                requests_total.labels(
                    provider=provider_name, engine=engine, method=op, status="error"
                ).inc()
                raise
            finally:
                request_duration_seconds.labels(
                    provider=provider_name, engine=engine, method=op
                ).observe(time.monotonic() - start)

            requests_total.labels(
                provider=provider_name, engine=engine, method="buyers", status="ok"
            ).inc()
            return record.model_dump(mode="json")
