...
(function () {
  ...
  // Contexto real de la compañía (si esta página es una ficha con CP_DATA/CE_DATA cargados)
  function buildCompanyContext() {
    try {
      var co = window.CP_DATA && window.CP_DATA.company;
      if (!co) return null;
      var fin = (window.CE_DATA && window.CE_DATA.finanzas && window.CE_DATA.finanzas.individual) || {};
      var parts = [];
      parts.push(co.name + (co.comercial ? ' ("' + co.comercial + '")' : ''));
      if (co.sector) parts.push('Sector: ' + co.sector + (co.cnae ? ' (CNAE ' + co.cnae + ')' : ''));
      if (co.location) parts.push('Ubicación: ' + co.location.city + ' (' + co.location.province + ')');
      if (fin.ventas) parts.push('Ventas: ' + fin.ventas);
      if (fin.ebitda) parts.push('EBITDA: ' + fin.ebitda + (fin.margen ? ' (margen ' + fin.margen + ')' : ''));
      if (fin.neto) parts.push('Beneficio neto: ' + fin.neto);
      if (co.employees) parts.push('Empleados: ' + co.employees);
      var r = window.CE_DATA && window.CE_DATA.radar;
      if (r && r.length) parts.push('Scores vs sector: ' + r.map(function (d) { return d.l + ' ' + d.p; }).join(', '));
      return { name: co.comercial || co.name, sector: co.sector, summary: parts.join('. ') };
    } catch (e) { return null; }
  }
  var companyCtx = buildCompanyContext();
  ...
  var presets = (function () {
    ...
    if (companyCtx) return ['Resume esta compañía', '¿Cuáles son sus riesgos?', '¿Quién la compraría?'];
  ...
  wrap.innerHTML =
    '<div class="cop" id="arrCop">' +
      ...
      '<div class="cop-panel"><div class="cop-msgs" id="arrMsgs">' +
        '<div class="cmsg bot"><span class="mav">✦</span><div class="mtxt">' + (companyCtx ? 'Hola, soy arroba copilot. Tengo cargado el análisis de <b>' + companyCtx.name + '</b>. ¿Qué te gustaría saber?' : 'Hola, soy arroba copilot. ¿En qué te ayudo en <b>' + pageName + '</b>?') + '</div></div>' +
      '</div></div>' +
  ...
  async function ask(q) {
    ...
    try {
      if (window.claude && window.claude.complete) {
        var sys = 'Eres arroba copilot, asistente de inteligencia económica de arroba.com (España). Te llamas "arroba copilot" (nunca "@Copilot" ni "Copilot" a secas). Tono de jefe de gabinete, conciso (3-5 frases), español, formato España (miles con punto, decimales con coma). Distingue dato recibido de inferencia y usa lenguaje probabilístico para lo inferido.';
        if (companyCtx) sys += ' Estás en la ficha de la compañía ' + companyCtx.name + '. Datos disponibles: ' + companyCtx.summary + '. Responde SIEMPRE usando estos datos cuando la pregunta sea sobre la compañía.';
        else sys += ' El usuario está en "' + pageName + '".';
        ans = await window.claude.complete(sys + ' Pregunta: ' + q);
  ...
  // Puentes de eventos: módulos de la ficha (AskAdvisor, etc.) piden foco/pregunta sin acoplarse al composer.
  window.addEventListener('arr-advisor-focus', function () { window.arrobaCopilot.focus(); });
  window.addEventListener('arr-advisor-ask', function (e) { window.arrobaCopilot.ask((e.detail && e.detail.q) || ''); });
})();
