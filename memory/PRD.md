...
> **Última actualización**: 2026-08-15 — **🟢 HARDENING-029 + HARDENING-032 CERRADOS**.
>
...
>
> **Verificación E2E** (preview pod, real Intel): `agencias de marketing` → 898 hits · 12 filas página 1 · chip growth ON → 3 filas (TLAXCA, PROMO RADIO, PRM INTERNACIONAL) · ambos toggles ON → mismas 3 (AND coherente). Total server-side y paginación intactos. `yarn tsc --noEmit` verde · `yarn build` verde · 229/230 unit tests verdes (el 1 fallo — `r14_comp_id_declaration_guard` sobre `atoms/MethodDetails/SrcDot/Tip`) es **pre-existente**, no tocado por este ticket).
>
> **Bundle final: 21 unidades** (19 previas + 2 nuevas). No deploy en este job.
>
