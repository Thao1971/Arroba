...
> **Última actualización**: 2026-08-14 — **🟢 HARDENING-REQ002 CERRADO**. Nueva página pública `/resultados?q=...` con tabla rica (Empresa · Ingresos · EBITDA · Crecim. · Score señales · Actualizado), chips filtro por sector, Exportar CSV activo, Comparar/Columnas "Próximamente", filas expandibles con 7 métricas + 4 acciones (solo "Abrir ficha" activa). El orchestrator redirige la búsqueda NL exploratoria a `/resultados` en vez de pintar `search_results` dentro del dock (`assistantMessage: "Te muestro los resultados para «...»"` + `navigate_to`). Preservados HARDENING-025/026 en `CopilotProvider` y en el test file (merge manual del test descartando la regresión del zip a UX pre-026 con FAB). Columnas financieras en «—» hasta que Intel emita `summary` en `SearchHit` (REQ enviado). Vitest 221/222 (única falla R14 pre-existente). Bundle final: **18 unidades**.
