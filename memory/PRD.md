...
> **Última actualización**: 2026-08-14 — **🟢 HARDENING-REQ001b + HARDENING-028 CERRADOS**. REQ001b añade búsqueda semántica NL vía Intel `POST /api/v1/semantic-intelligence/search` reutilizando el `AgencyToolClient` canónico (retry / dual-key / circuit breaker) + `WorkspaceArea` navega a `/empresa-f01/{cif}` al hacer click en un resultado. HARDENING-028 blinda `post_deploy.sh` con Paso 0 (rebuild+restart local del frontend) y Paso 6 (smoke retry backoff ~5 min contra `/api/platform/stats`). DEPLOY_NOTES.md destaca el push coordinado con Intel (re-embed). Bundle final: **17 unidades**. Backend pytest 22/22 + Vitest 54/54 verdes. Smoke funcional dev pod: query concreta B28184687 → resolve `/empresa-f01/`, query NL "clínicas dentales en Valencia" → semantic `total: 2` reales.
