...
> **Última actualización**: 2026-08-14 — **🟢 HARDENING-REQ001 CERRADO** (Intel real + hero search funcional + PlatformStats reshape 8→4 métricas). El bundle REQ-001 se aplicó tras refactor: se **descartó** el `intel_client.py` paralelo y ambos consumidores (`agency_tool_adapter.get_platform_stats` + `copilot._execute_search_real`) reutilizan el `AgencyToolClient` canónico del `intelligence_layer` (retry / dual-key / semáforo / circuit breaker). Toggle único `IntelligenceSettings.agency_tool_mode` (no se añadió `adapter_mode`). Rewrite `/empresa/` → `/empresa-f01/` en 6 sitios. `CopilotProvider.tsx` merge manual de 1 línea preservando HARDENING-025/026 intactos. Backend pytest 12/12 platform_stats + Vitest 104/104 verde. `provenance: live` confirmado end-to-end contra Intel real. Bundle final: **15 unidades** listas para push.
